...
pub(crate) fn decode_wrapped_table<R: Read>(rd: &mut R) -> Result<Vec<WrappedDek>, ArchiveError> {
    ...
    for _ in 0..wrapped_count {
        ...
        rd.read_exact(&mut ephemeral_public)
            .map_err(|_| ArchiveError::Parse)?;
        rd.read_exact(&mut nonce).map_err(|_| ArchiveError::Parse)?;
...
pub(crate) fn parse_part_file(
    ...
    let mut chunk_ciphertexts = Vec::with_capacity(chunk_count as usize);
    for _ in 0..chunk_count {
        let len = read_u32_be(&mut rd)? as usize;
        let mut ct = vec![0u8; len];
        rd.read_exact(&mut ct).map_err(|_| ArchiveError::Parse)?;
